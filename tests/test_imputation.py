...
import json
import pandas as pd
from moto import settings, mock_sqs, mock_sqs_deprecated, mock_sns
import sys, os
sys.path.append(os.path.realpath(os.path.dirname(__file__)+"/.."))
import calculate_means_wrangler, calculate_means_method 
...
class TestImputation:
  ...
  def test_data_output(self):
    ...
    with mock.patch.dict(calculate_means_method.os.environ, {
           ...
           'questions_list': 'movement_Q601_asphalting_sand movement_Q602_building_soft_sand movement_Q603_concreting_sand movement_Q604_bituminous_gravel movement_Q605_concreting_gravel movement_Q606_other_gravel movement_Q607_constructional_fill'
       }):
...
# Wrangler
    # with mock.patch('calculate_means_wrangler.boto3') as mocked:
    #   mocked.client.side_effect=Exception('SQS Failure')
    #   response = calculate_means_wrangler.lambda_handler("", None)
    #   assert 'success' in response
    #   assert response['success'] is False