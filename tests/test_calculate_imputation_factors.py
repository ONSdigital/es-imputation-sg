...
class test_wrangler_and_method(unittest.TestCase):
    ...
    def setup_class(cls):
        ...
        cls.mock_boto_wrangler_patcher = mock.patch('calculate_imputation_factors_wrangler.boto3')
        ...
        cls.mock_boto_method_patcher = mock.patch('calculate_imputation_factors_method.boto3')
    ...
    def test_method(self):
        ...
        input_file = "tests/calculate_imputation_factors_method_input_data.json"
        ...
        # final output match
        expected_output_file = "tests/calculate_imputation_factors_method_output.json"
        ...
        def test_method_exception_handling(self):
            ...
            json_data_content = '[{"movement_Q601_asphalting_sand":0.0},' \
                                '{"movement_Q601_asphalting_sand":0.857614899}]'

            response = calculate_imputation_factors_method.lambda_handler(json_data_content, None)
            assert not response['success']

        def test_wrangler_success_responses(self):
            """
            testing the wrangler success responses
            :param self:
            :return: mock response
            """

            with mock.patch('calculate_imputation_factors.json') as mock_json:
                response = calculate_imputation_factors_wrangler.lambda_handler(None, None)
                print(mock_json.dumps.called)
